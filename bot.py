...
def is_addressed(update, context):
    ...
    for name in BOT_NAMES:
        pattern = rf"^(?:[\W_]*)(?:{re.escape(name.lower())})(?:\b|[،,: ])"
...
async def teach_handler(update, context):
    ...
    if not question or not answers_list:
        await update.message.reply_text("سوال یا جواب‌ها مشکل دارن 😐")
        return

    TRAINING_DATA.append((question, answers_list))
    rebuild_model()

    await update.message.reply_text("یاد گرفتم، با چند جواب 😉")
...
async def message_handler(update, context):
    ...
    if answer:
        ...
        await update.message.reply_text("این رو هنوز یاد نگرفتم 🤔")

async def learn_on(update, context):
    global LEARNING_MODE
    LEARNING_MODE = True
    await update.message.reply_text("📚 حالت یادگیری لحن روشن شد")

async def learn_off(update, context):
    global LEARNING_MODE
    LEARNING_MODE = False
    await update.message.reply_text("🛑 حالت یادگیری لحن خاموش شد")

# Corrected indentation for show_samples function
async def show_samples(update, context):
    if not STYLE_MEMORY:
        await update.message.reply_text("چیزی یاد نگرفتم هنوز")
        return
